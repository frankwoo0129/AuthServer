...
var generateToken = function (type, req, callback) {
	...
	if (type === 'accessToken') {
		AccessToken.findOne({id: id}, function (err, result) {
			if (err) {
				callback(err);
			} else if (result) {
				setTimeout(function () {
		...
		RefreshToken.findOne({id: id}, function (err, result) {
			if (err) {
				callback(err);
			} else if (result) {
				setTimeout(function () {
...
module.exports.getUser = function (username, password, callback) {
	...
	User.findOne(query, function (err, result) {
		if (err) {
			...
			console.log(result.user + ' start strategy: ' + new Date());
			var authenticate = require('../strategy/' + result.strategy).authenticate;
			authenticate(result.user, password, function (err, auth) {
				console.log(result.user + ' end strategy: ' + new Date());
				if (err) {
					callback(err);
				} else if (!auth) {
					console.log('user: ' + result.user + ', ' + 'password: ' + password);
					callback({
						message: 'invalid_grant',
						status: 401
					});
